    ...
    <div id="services" className="w-full px-[12%] py-10 scroll-mt-20">
        ...
        <div className="grid auto-grid gap-6 my-10">
            {serviceData.map(({icon, title, description, link}, index: number) => (
                ...
                <div key={index} className="border border-gray-400 rounded-lg px-8 py-12 hover:bg-lighthover hover:shadow-black duration-500 cursor-pointer hover:-translate-y-1">
                    <Image src={icon} alt={title} className="w-7 mt-3" />
                    <h3 className="text-lg text-gray-700 my-3">{title}</h3>
                    <p className="text-sm text-gray-600 leading-5 mt-3">{description}</p>
                    <a href={link} className="flex items-center justify-end text-sm text-gray-600 leading-5 mt-3  gap-2">Read more
                    <Image src={assets.arrow_icon} alt="arrow-right" className="w-4" />
                    </a>
                </div>
            ))}
